...
async function loginUser(page, user) {
    ...
    // Click LOGIN via JS (span is hidden in DOM)
    await page.evaluate(() => {
        const spans = document.querySelectorAll('span.primary-button.button-login');
        if (spans.length > 0) spans[0].click();
    });

    // Wait for Kabam login page to load (full page navigation, no popup/iframe)
    await page.waitForURL(/kabam|oauth|auth/i, { timeout: 15000 });
